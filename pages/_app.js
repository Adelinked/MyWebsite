...
function ClientSideTheme() {
  ...
  useEffect(() => {
    let storedTheme =
      JSON.parse(localStorage.getItem("Adelinked"))?.theme ?? "dark";
    let storedDisplay =
      JSON.parse(localStorage.getItem("Adelinked"))?.display ?? "0";
    let storedShowCmd =
      JSON.parse(localStorage.getItem("Adelinked"))?.showCmd ?? false;
    let storedFixNavBar =
      JSON.parse(localStorage.getItem("Adelinked"))?.fixNavBar ?? false;
    if (!globalState) {
